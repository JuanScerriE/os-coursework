...
A list of test input can be found at
\texttt{src/task3and4/tests.txt}. There are ten test inputs.
They are all identified as being either valid or invalid.
...
\item
  ...
  with allowing the shell to conform to the POSIX standard more
  easily. Currently, the behaviour of \texttt{tish} is different
  from \texttt{bash}. For example the following command
  \texttt{figlet | sed "s/ /*/g" \textless\ infile.txt} is
  different in \texttt{tish} because of the fact that
  \texttt{infile.txt} is being bound to the \texttt{stdin} of
  \texttt{figlet} instead of that of \texttt{sed}. The fact that
  a single function is being used to execute a pipeline, makes
  it very difficult to implement since information about where
  the redirection occurred is crucial to bind the \texttt{stdin}
...
\item
  ...
  variables. Instead it would be better for functions which
  require multiple return types or multiple inputs to be
\end{itemize}
