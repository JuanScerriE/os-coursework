...
calls and handles each of the respective functions described
...
%   different in \texttt{tish} because of the fact that the
%   \texttt{infile.txt} is bound to the \texttt{stdin} of
%   \texttt{figlet} instead of that of \texttt{sed}. The fact that
%   a single function is being used to execute a pipeline, makes
%   it very difficult to implement such a feature since
%   information about where the redirection occurred is crucial to
%   bind the \texttt{stdin} of the preceding command.
\item
